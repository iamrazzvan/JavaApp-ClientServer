...
import com.persistence.utils.JDBCUtils;
...
public class ParticipantDBRepository implements IParticipantRepository {
    ...
    public Collection<Participant> getParticipantsByTeam(Long teamID) {
        logger.traceEntry("getParticipantsByTeam with task {} ", teamID);
    ...
    public Optional<Participant> getParticipantByData(Participant participant) {
        logger.traceEntry("getParticipantByData with task {} ", participant);
        Connection connection = dbUtils.getConnection();
        try (PreparedStatement preparedStatement = connection.prepareStatement(
                "SELECT * FROM participants WHERE firstname=? AND lastname=? AND engineCc=? AND teamID=?")) {
            setParams(participant, preparedStatement);
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                if (resultSet.next()) {
                    Participant extracted = extract(resultSet);
                    logger.traceExit(extracted);
